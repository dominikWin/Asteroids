...
public class World {
    ...
    public void update() {
	if (Math.random() < getAsteroidAddChance() && asteroids.size() < MAX_ASTEROID_COUNT)
	...
	while(true) {
	    Vector2d loc = new Vector2d(Math.random() * (double) Game.WIDTH, Math.random() * (double) Game.HEIGHT);
	    if(Vector2d.distance(loc, getPlayer().getLocation()) < MIN_ASTEROID_DISTANCE_TO_PLAYER) return loc;
	}
    }
