...
public class Progress {
	...
	for (int i = 0; i < getNextRoundAsteroidCount(); i++) {
	    Game.getInstance().getWorld()
		    .addAsteroid(new Asteroid(Asteroid.getRandomLocation()));
	}
    }
	...
	case WAIT_PLAYER:
		...
		Game.getInstance().getUi()
			.setMessage("Fast gun unlocked!", UNLOCK_MESSAGE_TIME);
	    }
		...
		Game.getInstance().getUi()
			.setMessage("Dual gun unlocked!", UNLOCK_MESSAGE_TIME);
	    }
		...
		Game.getInstance()
			.getUi()
			.setMessage("Tripple gun unlocked", UNLOCK_MESSAGE_TIME);
	    }
		...
		Game.getInstance().getUi()
			.setMessage("Quad gun unlocked", UNLOCK_MESSAGE_TIME);
	    }
