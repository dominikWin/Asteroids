...
public class DistortedPolygon extends Polygon {
	...
	for (int i = 0; i < sides; i++) {
	    distances.add(new Double((double) radius * Math.random() * (2-.5) + .5));
	}
    }
	...
	distances = new ArrayList<>();
	for (int i = 0; i < sides; i++) {
	    distances.add(new Double((double) radius * Math.random() * (maxChange-minChange) + minChange));
	}
    }
	...
	double innerAngle = 360d / (double) super.getSides();
	for (int i = 0; i < super.getSides(); i++) {
	    getPoints().add(
		    new Vector2d(super.getLocation(), innerAngle * (double) i
